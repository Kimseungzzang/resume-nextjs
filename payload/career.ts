        ...
        },
        {
          ...
          blocks: [
            ...
            { type: 'heading', text: '해결 방법' },
            {
              type: 'paragraph',
              text:
                '@TransactionalEventListener(AFTER_COMMIT) + @Async 조합을 도입했습니다. AFTER_COMMIT 리스너는 데이터 저장이 완전히 완료된 시점에만 실행되므로, 저장에 실패하면 메일 발송 이벤트 자체가 실행되지 않습니다. 여기에 @Async를 결합해 메일 발송을 별도 스레드에서 처리하도록 분리해, 메일 발송이 수검자 등록 응답 속도에 영향을 주지 않게 했습니다.',
            },
            {
              type: 'paragraph',
              text:
                '이벤트 발행 시점은 수검자 번호가 최종 확정되는 마지막 저장 단계 완료 후로 변경해, 수검번호가 빈 값으로 발송되는 문제를 해결했습니다. SecurityContext 문제는 등록 시점(요청 스레드)에 담당자 ID를 미리 추출해 이벤트에 함께 전달하고, 비동기 스레드에서는 이 ID로 이메일 주소를 조회하는 방식으로 해결했습니다.',
            },
            {
              type: 'paragraph',
              text:
...
최종 답변: 수검자 1031번은 2026-06-10에 검사를 완료했고 점수는 87점입니다.`,
  ...
  personalProjects: [
    {
      title: 'AI 자율 트레이딩 시스템 (Jarvis)',
      href: 'https://github.com/Kimseungzzang/quant',
      description:
        '증권사 API와 LLM Tool-Use를 결합해, AI가 시세·지표를 분석해 감시 조건을 직접 설정하고 조건 충족 시 스스로 매매까지 판단·실행하는 자율 트레이딩 에이전트입니다. FastAPI + asyncio 단일 이벤트 루프로 실시간 시세 수신과 지표 계산, 조건 평가를 동시에 처리하도록 구현했습니다.',
    },
    {
      title: 'Claude Code 세션 유지 파이프라인',
      href: 'https://github.com/Kimseungzzang/zzang-claude-skills',
      description:
        '여러 기기에서 Claude Code 작업을 이어갈 수 있도록, 작업 로그를 자동 기록하고 세션 컨텍스트를 GitHub 저장소에 push/pull하는 커스텀 스킬 모음을 제작했습니다.',
    },
  ],
};
