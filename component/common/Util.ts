...
enum LUXON_DATE_FORMAT {
  ...
  DURATION_KINDNESS_EN = "y'y 'M'm'",
  DURATION_KINDNESS_ONLY_MONTH_EN = "M'm'",
  DURATION_KINDNESS_ONLY_YEAR_EN = "y'y'",
}

function getFormattingDuration(from: DateTime, to: DateTime = DateTime.local(), lang: Lang = 'ko') {
  ...
  if (diff.years > 0 && diff.months === 0) {
    format = isEn
      ? LUXON_DATE_FORMAT.DURATION_KINDNESS_ONLY_YEAR_EN
      : LUXON_DATE_FORMAT.DURATION_KINDNESS_ONLY_YEAR;
  } else if (diff.years === 0 && diff.months > 0) {
    // 1년 미만이면 개월만 표시
    format = isEn
      ? LUXON_DATE_FORMAT.DURATION_KINDNESS_ONLY_MONTH_EN
      : LUXON_DATE_FORMAT.DURATION_KINDNESS_ONLY_MONTH;
  } else {
    format = isEn ? LUXON_DATE_FORMAT.DURATION_KINDNESS_EN : LUXON_DATE_FORMAT.DURATION_KINDNESS;
  }
